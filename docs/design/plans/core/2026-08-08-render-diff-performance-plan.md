...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:test-driven-development for the `coding` tasks and superpowers:executing-plans to drive the plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
- [ ] **Step 1 (RED): Add inline `#[cfg(test)]` tests** in
...
- [ ] **Step 2 (GREEN): Implement `pub fn render_diff(writer: &mut impl
...
- [ ] **Step 3 (REFACTOR): Rewrite `draw_diff`** to
...
- [ ] **Step 1: Change `out: Stdout` to `out: BufWriter<Stdout>`,**
...
- [ ] **Step 2: Confirm `draw_diff`'s single `self.out.flush()`** now
...
- [ ] **Step 1: Add to `Cargo.toml`** `criterion = "0.5"` under
  `[dev-dependencies]` and a `[[bench]]` entry
  (`name = "render"`, `harness = false`).
- [ ] **Step 2: Write `benches/render.rs`** that builds a source
...
- [ ] **Step 3: Add a private `render_diff_naive`** in the bench file
...
- [ ] `cargo test` green, including the new `render_diff` coalescing
  tests (the two `#[ignore]`d real-TTY tests unchanged).
- [ ] `cargo clippy --all-targets -- -D warnings` and
  `cargo fmt --check` clean.
- [ ] `cargo build --examples` — all example apps compile against the
  unchanged `draw_diff`.
- [ ] `cargo bench` runs and shows `render_diff` doing meaningfully less
